...
import { getCharacterCanon, getCharacterGrowth, getCharacterViewById } from "@/features/characters";
...
export default function CharacterHomeScreen() {
  ...
  const storyEvent = stories.getActiveEvent(id);
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {storyEvent && storyState && <View style={[styles.storyCard,storyState.locked&&styles.storyLocked]}>
        <View style={styles.storyTop}><View style={{flex:1}}><Text style={styles.storyEyebrow}>{storyState.claimed?text("PAST EVENT","往期事件"):storyState.locked?text("UPCOMING STAGE EVENT","未来阶段事件"):text("CURRENT STAGE EVENT","当前阶段事件")}</Text><Text style={styles.storyTitle}>{language==="zh"?storyEvent.titleZh:storyEvent.titleEn}</Text><Text style={styles.storyStage}>{text(`Main life path · Bond Lv.${storyState.requiredBondLevel}`,`人生主线阶段 · 好感 ${storyState.requiredBondLevel} 级`)}</Text></View><Text style={styles.storyProgress}>{storyState.locked?"🔒":`${storyState.completedCount}/${storyState.objectives.length}`}</Text></View>
        <Text style={styles.storyPremise}>{language==="zh"?storyEvent.premiseZh:storyEvent.premiseEn}</Text>
        {!storyState.locked&&<View style={styles.objectiveList}>{storyState.objectives.map((objective,index)=><View key={`${objective.type}-${index}`} style={styles.objective}><Text style={styles.objectiveCheck}>{objective.complete?"✓":"○"}</Text><Text style={styles.objectiveText}>{language==="zh"?objective.labelZh:objective.labelEn}</Text><Text style={styles.objectiveCount}>{objective.current}/{objective.target}</Text></View>)}</View>}
        <Text style={styles.storyMailHint}>{storyState.locked?text(`This optional event becomes available when the main life path reaches Bond Lv.${storyState.requiredBondLevel}.`,`这是穿插在主线中的可选事件，好感达到 ${storyState.requiredBondLevel} 级后开放。`):text("Each completed objective delivers an event letter. Events enrich the current life stage without replacing the main life path.","每完成一项目标都会收到事件来信；事件用于丰富当前人生阶段，不会取代人物主线。")}</Text>
        {!storyState.locked&&<Pressable disabled={!storyState.completed||storyState.claimed} onPress={()=>stories.claim(storyEvent)} style={[styles.claimButton,(!storyState.completed||storyState.claimed)&&styles.claimDisabled]}><Text style={styles.claimText}>{storyState.claimed?text("Reward claimed","奖励已领取"):storyState.completed?text(`Claim ${storyEvent.rewardBubbles} Bubble · ${storyEvent.rewardBond} Bond`,`领取 ${storyEvent.rewardBubbles} Bubble · ${storyEvent.rewardBond} 好感`):text("Complete the event objectives to claim","完成事件目标后领取")}</Text></Pressable>}
      </View>}
