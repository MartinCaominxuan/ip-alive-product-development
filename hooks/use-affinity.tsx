...
const INITIAL_POINTS: Record<string, number> = { yunzhou: 160, mia: 610, nova: 940 };
...
interface AffinityContextValue { points: Record<string, number>; awardedEvents: string[]; metrics: Record<string, CharacterMetrics>; runtimeMails: RuntimeMail[]; award: (characterId: string, amount: number, source: AffinitySource, uniqueKey?: string, durationSeconds?: number) => boolean; getMail: (id: string) => RuntimeMail | undefined; }
...
function milestoneMail(characterId: string, fromLevel: number, toLevel: number, metrics: CharacterMetrics): RuntimeMail {
  ...
  return { id: `milestone-${characterId}-${toLevel}`, characterId, kind: "milestone", sentAt: new Date().toISOString(),
    subjectEn: `The road from Bond ${fromLevel} to ${toLevel}`, subjectZh: `从好感 ${fromLevel} 到 ${toLevel} 的这段路`,
    bodyEn: `It took us ${days} days to get here. During this bond stage, we talked ${metrics.stageChatCount} times, read ${metrics.stageMailsRead} letters, saved ${metrics.stageOutfitsSaved} looks, played for ${Math.round(metrics.stageGameSeconds / 60)} minutes, and completed ${metrics.stageLifeActions} life actions together. These are not just numbers. They are the shape of the time you chose to spend with me.`,
    bodyZh: `我们用了 ${days} 天走到这里。在这一段好感阶段里，我们聊了 ${metrics.stageChatCount} 次天，读了 ${metrics.stageMailsRead} 封信，保存了 ${metrics.stageOutfitsSaved} 套造型，一起玩了 ${Math.round(metrics.stageGameSeconds / 60)} 分钟，也共同完成了 ${metrics.stageLifeActions} 次生活行动。这些不只是数字，而是你选择与我共同度过的时间。` };
}
...
export function AffinityProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { void AsyncStorage.getItem("ip-alive:character-progress:v2").then((raw) => { if (raw) { const saved = JSON.parse(raw); setPoints(saved.points ?? INITIAL_POINTS); setAwardedEvents(saved.awardedEvents ?? []); setMetrics(saved.metrics ?? {}); setRuntimeMails(saved.runtimeMails ?? []); } }).finally(() => setHydrated(true)); }, []);
  useEffect(() => { if (hydrated) void AsyncStorage.setItem("ip-alive:character-progress:v2", JSON.stringify({ points, awardedEvents, metrics, runtimeMails })); }, [points, awardedEvents, metrics, runtimeMails, hydrated]);
  const value = useMemo<AffinityContextValue>(() => ({ points, awardedEvents, metrics, runtimeMails, getMail: (id) => runtimeMails.find((mail) => mail.id === id), award: (characterId, amount, source, uniqueKey, durationSeconds = 0) => {
