...
export function countClearedColors(board: Array<MatchCell | null>, clear: Set<number>) { const result: Partial<Record<Match3Tile, number>> = {}; clear.forEach((index) => { const color = board[index]?.color; if (color) result[color] = (result[color] ?? 0) + 1; }); return result; }
